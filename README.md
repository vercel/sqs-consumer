...
* `heartbeatInterval` - _Number_ - The interval (in seconds) between requests to extend the message visibility timeout. On each heartbeat the visibility is extended by adding `visibilityTimeout` to the number of seconds since the start of the handler function. This value must less than `visibilityTimeout`. If a heartbeat visibility update fails, the consumer emits an `error` event and stops sending further heartbeats for that message or batch.
